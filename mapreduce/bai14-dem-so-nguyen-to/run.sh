#!/usr/bin/env bash
# Bài 14 – Đếm số nguyên tố: biên dịch, chạy trên Hadoop Standalone và kiểm tra kết quả.
#
...
jar cf "$HERE/build/bai14.jar" -C "$HERE/build/classes" .
...
time hadoop --config "$CONF" jar "$HERE/build/bai14.jar" bigdata.bai14.PrimeCount "$INPUT" "$OUTPUT"
